...
The PRG file format is a container-based format for storing diagrams created in the [**Project Graph**](/docs/prg) application. It leverages the ubiquitous ZIP archive format to bundle a primary serialized graph description (`stage.msgpack`) with its associated binary attachments (images, documents, etc.).
